...
public:
	 ...
	 */
	BabySitter(const ros::NodeHandle &nh, BagLogger *logger, const std::string &node_name,
			const std::string &topic, int warn_ms, int error_ms, int warn_count_thresh = 5,
			int timeout_ms = 2000, int start_delay_ms = 0 /*, ErrorCB error_cb */);
...
void BabySitter<M>::deviceCB(const ros::MessageEvent<M const>& event)
{
	...
	{
		ROS_ERROR_STREAM_THROTTLE(1.0, NODE_FUNC << node_name_ << ": max latency error: " << latency_ms <<
				"(" << error_ms_ << ")");
		device_state_ = DeviceState::ERROR;
	}
	else if(latency_ms >= warn_ms_)
	{
		ROS_WARN_STREAM_THROTTLE(1.0, NODE_FUNC << node_name_ << ": latency warning: " << latency_ms <<
				"(" << warn_ms_ << ")");
...
void BabySitter<M>::heartbeatCB(const ros::TimerEvent& event)
{
	...
	ROS_INFO_STREAM_THROTTLE(LOG_PERIOD_S, NODE_FUNC << node_name_ <<
			" node:" << parseNodeState(node_state_) <<
