...
public:
    ...
    virtual LifeCycleStatus process()
    {
        ...
        {
            ROS_ERROR_STREAM_THROTTLE(1.0, long_name_ << " exceeding max_error: " <<
                    value_ << " (max:" << max_error_ << ")");
            compoundStatus(status, LifeCycleStatus::ERROR);
        }
        else if(value_ > max_warn_)
        {
            ROS_WARN_STREAM_THROTTLE(1.0, long_name_ << " exceeding max_warn: "<<
                    value_ << " (max:" << max_warn_ << ")");
