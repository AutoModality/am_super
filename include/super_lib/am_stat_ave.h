...
public:
    ...
    AMStatAve(const std::string &short_name, const std::string &long_name,
            uint32_t max_warn, uint32_t max_error) : AMStatReset(short_name, long_name, max_warn, max_error)
    {
    }

    AMStatAve(const std::string &short_name, const std::string &long_name,
            uint32_t min_error, uint32_t min_warn, uint32_t max_warn, uint32_t max_error) :
    ...
    AMStatAve(const std::string &short_name, const std::string &long_name,
            uint32_t min_error, uint32_t min_warn, uint32_t max_warn, uint32_t max_error,
            uint32_t min_min_error = 0,
            uint32_t min_min_warn = 0,
    ...
    LifeCycleStatus process() override
    {
        ...
        if(ave > max_error_)
        {
            ROS_ERROR_STREAM_THROTTLE(1.0, long_name_ << " ave exceeding max_error: " <<
                    ave << " (max:" << max_error_ << ")");
            compoundStatus(status, LifeCycleStatus::ERROR);
        }
        else if(ave > max_warn_)
        {
            ROS_WARN_STREAM_THROTTLE(1.0, long_name_ << " ave exceeding max_warn: " <<
                    ave << " (max:" << max_warn_ << ")");
        ...
        if(ave < min_error_)
        {
            ROS_ERROR_STREAM_THROTTLE(1.0, long_name_ << " ave exceeding min_error: " <<
                    ave << " (min:" << min_error_ << ")");
            compoundStatus(status, LifeCycleStatus::ERROR);
        }
        else if(ave < min_warn_)
        {
            ROS_WARN_STREAM_THROTTLE(1.0, long_name_ << " ave exceeding min_warn: " <<
                    ave << " (min:" << min_warn_ << ")");
            compoundStatus(status, LifeCycleStatus::WARN);
        }
        ...
        {
            ROS_ERROR_STREAM_THROTTLE(1.0, long_name_ << " min exceeding min_min_error: " <<
                    min_ << " (min:" << min_min_error_ << ")");
            compoundStatus(status, LifeCycleStatus::ERROR);
        }
        else if(min_ < min_min_warn_)
        {
            ROS_WARN_STREAM_THROTTLE(1.0, long_name_ << " min exceeding min_min_warn: " <<
                    min_ << " (min:" << min_min_warn_ << ")");
            compoundStatus(status, LifeCycleStatus::WARN);
        }

        if(max_ >max_max_error_)
        {
            ROS_ERROR_STREAM_THROTTLE(1.0, long_name_ << " max exceeding max_max_error: " <<
                    max_ << " (max:" << max_max_error_ << ")");
            compoundStatus(status, LifeCycleStatus::ERROR);
        }
        else if(max_ >max_max_warn_)
        {
            ROS_WARN_STREAM_THROTTLE(1.0, long_name_ << " max exceeding max_max_warn: " <<
                    max_ << " (max:" << max_max_warn_ << ")");
            compoundStatus(status, LifeCycleStatus::WARN);
        }
    ...
    uint32_t getAve()
    {
        ...
        uint32_t ave_32 = ave_64 > std::numeric_limits<uint32_t>::max() ?
    ...
    std::string getStrShort() override
    {
        std::stringstream ss;
        ss <<
                short_name_ << "-av:" << getAve() <<
                "," << short_name_ << "-mx:" << getMax() <<
                "," << short_name_ << "-mn:" << getMin();
        return ss.str();
    }
    ...
    std::string getStr() override
    {
        std::stringstream ss;
        ss <<
                long_name_ << " Ave: " << getAve() <<
                "," << long_name_ << " Max: " << getMax() <<
                "," << long_name_ << " Min: " << getMin();
        return ss.str();
    }
    ...
    void setMaxMaxWarn(uint32_t maxMaxWarn =
            std::numeric_limits<uint32_t>::max())
    {
