...
public:
    AMStatReset(const std::string &short_name, const std::string &long_name) :
    ...
    LifeCycleStatus process() override
    {
        ...
        {
            ROS_ERROR_STREAM_THROTTLE(1.0, long_name_ << " exceeding min_error: " <<
                    value_ << " (min:" << min_error_ << ")");
            compoundStatus(status, LifeCycleStatus::ERROR);
        }
        else if(value_ < min_warn_)
        {
            ROS_WARN_STREAM_THROTTLE(1.0, long_name_ << " exceeding min_warn: " <<
                    value_ << " (min:" << min_warn_ << ")");
