...
namespace am
{
    ...
    bool
    StateMediator::allowsTransition(SuperState from, SuperState to)
    {
        bool legal = false;
        switch (from)
        {
        ...
        case SuperState::AUTO:
            if (to == SuperState::READY || to == SuperState::SEMI_AUTO || to == SuperState::HOLD ||
                to == SuperState::ABORT || to == SuperState::MANUAL)
            {
                legal = true;
            }
            break;
        case SuperState::SEMI_AUTO:
            if (to == SuperState::AUTO || to == SuperState::HOLD || to == SuperState::ABORT ||
