...
public:
  Super() : nh_("~")
  {
    ...
    ROS_INFO_STREAM(NODE_FUNC << "status_log_level = " << status_log_level_);
      ...
      for (string& name : manifest_)
      {
        ...
        if (!name.compare("os1_node"))
        {
          ...
          lidar_bs_ = new am::BabySitter<sensor_msgs::PointCloud2>(
              nh_, BagLogger::instance(), name, am::am_topics::SENSOR_LIDAR_POINTCLOUD2, lidar_warn_ms, lidar_error_ms,
              5, 2000, 10000 /*, std::bind(&Super::errorCB, this) */);
        }
        else if (!name.compare("can_node"))
        {
          int altimeter_warn_ms, altimeter_error_ms;
          calcTiming(ALTIMETER_HZ, altimeter_warn_ms, altimeter_error_ms);
          altimeter_bs_ = new am::BabySitter<brain_box_msgs::StampedAltimeter>(
              nh_, BagLogger::instance(), name, "/sensor/distance/agl_lw", altimeter_warn_ms, altimeter_error_ms);
        }
        else if (!name.compare("dji_sdk"))
        {
          int dji_warn_ms, dji_error_ms;
          calcTiming(RC_HZ, dji_warn_ms, dji_error_ms);
          dji_bs_ = new am::BabySitter<sensor_msgs::Joy>(nh_, BagLogger::instance(), name, "/dji_sdk/rc", dji_warn_ms,
                                                         dji_error_ms);
        }
      }
    }
    ...
    // lidar_sub_ = nh_.subscribe<sensor_msgs::PointCloud2>(am::am_topics::SENSOR_LIDAR_POINTCLOUD2, 10,
    // &Super::lidarPC2CB, this);

    heartbeat_timer_ = nh_.createTimer(ros::Duration(1.0), &Super::heartbeatCB, this);
  }
  ...
  void processState(const std::string& node_name_in, const am::LifeCycleState state, const std::string& status,
                    const std::string& value, const int pid, const ros::Time& last_contact)
  {
  ...
  void heartbeatCB(const ros::TimerEvent& event)
  {
    ...
    else
    {
      ...
      for (it = nodes_.begin(); it != nodes_.end(); it++)
      {
        if (it->second.running)
        {
          ros::Duration time_since_contact = now - it->second.last_contact;
          ...
          if (time_since_contact > timeout_dur)
          {
            ...
            ROS_ERROR_STREAM(NODE_FUNC << "node timed out:" << it->second.name);
            printStatus();
          }
        }
      }
    ...
    std::stringstream ss;
    ss << NODE_FUNC << "state: " << parseState(state_) << ", man: " << manifest_.size()
       << ", man run:" << man_run_nodes_ << ", run: " << run_nodes_;
    ...
    for (it = nodes_.begin(); it != nodes_.end(); it++)
    {
      if (!it->second.running)
      {
        ROS_ERROR_STREAM(NODE_FUNC << "node not running:" << it->second.name);
      }
    }
  }
  ...
  void printStatus()
  {
    ROS_INFO_STREAM(NODE_FUNC << "man: " << manifest_.size() << ", man run:" << man_run_nodes_
                              << ", run: " << run_nodes_);
  }
  ...
  {
    ROS_INFO_STREAM(NODE_FUNC << "changing state from: " << parseState(state_) << " to: " << parseState(state));
    switch (state_)
    {
      case brain_box_msgs::VxState::BOOTING:
        state_ = state;
