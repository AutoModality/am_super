	...
	if(tmp_manifest.size())
	{
		...
		for(string &name: manifest_)
		{
			...
			if(!name.compare("os1_node"))
			{
				...
				lidar_bs_ = new am::BabySitter<sensor_msgs::PointCloud2>(nh_, BagLogger::instance(), name,
						am::am_topics::SENSOR_LIDAR_POINTCLOUD2, lidar_warn_ms, lidar_error_ms, 5, 2000, 10000 /*, std::bind(&Super::errorCB, this) */);
			}
			else if(!name.compare("can_node"))
			{
				...
				altimeter_bs_ = new am::BabySitter<brain_box_msgs::StampedAltimeter>(nh_, BagLogger::instance(), name,
						"/sensor/distance/agl_lw", altimeter_warn_ms, altimeter_error_ms);
			}
			else if(!name.compare("dji_sdk"))
			{
				int dji_warn_ms, dji_error_ms;
				calcTiming(RC_HZ, dji_warn_ms, dji_error_ms);
				dji_bs_ = new am::BabySitter<sensor_msgs::Joy>(nh_, BagLogger::instance(), name,
						"/dji_sdk/rc", dji_warn_ms, dji_error_ms);
			}
		}
	}
	printStatus();
...
void heartbeatCB(const ros::TimerEvent& event)
{
	...
	std::stringstream ss;
	ss << NODE_FUNC << "state: " << parseState(state_) <<
			", man: " << manifest_.size() << ", man run:" << man_run_nodes_ << ", run: " << run_nodes_;
...
void printStatus()
{
	ROS_INFO_STREAM(NODE_FUNC << "man: " << manifest_.size() << ", man run:" << man_run_nodes_ << ", run: " << run_nodes_);
}
