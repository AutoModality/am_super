        ...
        nh_("~")
{
     ...
     */
    lifecycle_sub_ = nh_.subscribe("/node_lifecycle", 100, &AMLifeCycle::lifecycleCB,
        this);
...
void AMLifeCycle::lifecycleCB(
    const brain_box_msgs::LifeCycleCommand::ConstPtr msg)
{
  ROS_DEBUG_STREAM_THROTTLE(1.0,
      commandToString((LifeCycleCommand )msg->command));

  if (!msg->node_name.compare(AMLifeCycle::BROADCAST_NODE_NAME)
      || !msg->node_name.compare(node_name_))
  {
    // if this message is for us
    switch ((LifeCycleCommand) msg->command)
    {
    case LifeCycleCommand::ACTIVATE:
      transition("activate", LifeCycleState::INACTIVE,
          LifeCycleState::ACTIVATING, LifeCycleState::ACTIVE,
          std::bind(&AMLifeCycle::onActivate, this));
      break;
    case LifeCycleCommand::CLEANUP:
      transition("cleanup", LifeCycleState::INACTIVE,
          LifeCycleState::CLEANING_UP, LifeCycleState::UNCONFIGURED,
          std::bind(&AMLifeCycle::onCleanup, this));
      break;
    case LifeCycleCommand::CONFIGURE:
      transition("configure", LifeCycleState::UNCONFIGURED,
          LifeCycleState::CONFIGURING, LifeCycleState::INACTIVE,
    ...
    case LifeCycleCommand::DEACTIVATE:
      transition("deactivate", LifeCycleState::ACTIVE,
          LifeCycleState::DEACTIVATING, LifeCycleState::INACTIVE,
...
void AMLifeCycle::transition(std::string transition_name, LifeCycleState initial_state,
    LifeCycleState transition_state,  LifeCycleState final_state, std::function<void(void)> on_function)
{
...
void AMLifeCycle::doTransition(std::string transition_name,
    bool success, LifeCycleState success_state, LifeCycleState failure_state)
{
...
{
  if(state_ == LifeCycleState::ERROR_PROCESSING || state_ == LifeCycleState::FINALIZED || state_ == LifeCycleState::UNCONFIGURED)
  {
...
void AMLifeCycle::heartbeatCB(const ros::TimerEvent &event)
{
    ...
    std::stringstream ss;
    ss << AMLifeCycle::stateToString(state_) <<
            "," << AMLifeCycle::statusToString(status_) << "," <<
